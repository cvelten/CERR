...
try
	...
	warning(strcat("Error encountered while calling 'getRasterSegs(planC)': ", ME.message));
end
try
	planC = setUniformizedData(planC);
catch
	warning(strcat("Error encountered while calling 'setUniformizedData(planC)': ", ME.message));
end
